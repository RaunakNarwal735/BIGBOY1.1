...
def parse_args():
    ...
    parser.add_argument("--incubation_days", type=int, default=3, help="Days between exposure and becoming infectious.")
...
def convert_real_data(args):
    ...
    # Approximate Exposed using a shift of incubation_days
    df['Exposed'] = df['new_cases'].shift(args.incubation_days).fillna(0).astype(int)
    ...
    bigboy_df = df[['Day', 'Susceptible', 'Exposed', 'Infected', 'Recovered', 'Reported_Cases', 'Beta_Effective', 'Season_Index']]
